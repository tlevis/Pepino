...
var targetEndpointI2C = {
	...
	dropOptions:{   hoverClass:"i2c-hover", 
                    activeClass:"i2c-active", 
                    activate: function( event, ui ) { 
                                    $('.i2c-description').css({bottom: 50});
                              },
                    deactivate: function( event, ui ) { 
                                    $('.i2c-description').css({bottom: 20});
                              }
                },
    parameters: {
		port: "i2c",
...
ObjectBoard.prototype.Draw  = function(offset, working_area) {
    ...
    body += "<div style='position: absolute; top: 80px; left: 120px; color: white; font-size: 10px'>~ - PWM<br/>* - Interrupt</div>";
    ...
    for (var idx in DigitalPorts)
    {
        ...
        if (targetConnection.parameters.isPWM == true) extraInfo += " ~";
        targetConnection.parameters.isInterrupt = (InterruptPorts.indexOf(DigitalPorts[idx]) != -1);
        if (targetConnection.parameters.isInterrupt == true) extraInfo += " *";
        ...
        body += "<div class='digital-numbers' style='color: white; position: absolute; top: 20px; left: " + pos + "px'>" + DigitalPorts[idx] + "<span style='font-size: 10px'>" + extraInfo + "</span></div>";        
    }	

	// i2c    
    body += "<div class='i2c-description'>i²c</div>";    
    sourceUUID = this.Id + "_InConnectionI2C_" + idx;
